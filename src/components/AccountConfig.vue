<template>
    <div>
                                ...
                                <v-btn
                                    ...
                                    @click="
                                        () =>
                                            $router.push({
                                                path: '/login',
                                                query: { ...account, canEditType: false }
                                            })
                                    "
                                >
        ...
        <CommonDialog
            ...
            width="500"
        >
            <v-btn text color="green" @click="() => (visible = false)">{{
                $t("Cancel")
            }}</v-btn>
            <v-btn text color="red" @click="delete_account">{{
                $t("Confirm")
            }}</v-btn>
        </CommonDialog>
    </div>
...
export default {
    ...
    watch: {
        selected: function(newSelected) {
            this.config.currentAccount = this.config.accounts.find(
                account => account.id === newSelected
            );
    ...
    methods: {
        show_delete_account(target) {
            this.deleteText = `${this.$t("DoYouReallyWantToDeleteThis")}(${
                target.username
            })`;
        ...
        delete_account() {
            this.config.accounts = this.config.accounts.filter(
                account => account.id !== this.deleteTarget.id
            );
