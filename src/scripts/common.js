...
function checkRules(rules) {
    ...
    for (let index = 0; index < rules.length; index++) {
        ...
        if (
            ...
            Object.values(rule.os).includes(system)
        ) {
            result = false;
        }
    }
...
async function scheduleDownloadTasks(props) {
    const { store, name, tasks, callback, onProgress: _onProgress } = props;
    console.log(tasks);
    const mission = {
        id: v4(),
        state: "Pending",
    ...
    const { cancel: cancelRequest, retry: retryMission } = patchDownload(
        tasks,
        {
            onProgress(progress) {
                ...
                store.commit("updateDownloadMission", {
                    id: mission.id,
                    ...progress
                });
            }
